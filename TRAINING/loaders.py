...
from utils import join_slot, shuffle_dataset, create_eval_group, get_transformations, name_csv_files
from custom_datasets import CustomDataset_vision, CustomDataset_vision_with_other, CustomDataset_vision_audio, CustomDataset_audio, CustomDataset_pose, CustomDataset_audio_cnn
import os
import torch
import numpy as np
import random
...
def data_loader_training(d, slot_folders, csv_training, test_filename):
    ...
    data_transform_face, data_transform_pose, data_transform_spect = get_transformations()
    ...
    if d['model'] == 'cnn_lstm_mm':
        ...
        dataset = {x: CustomDataset_vision_with_other(label_dir=os.path.join(d['data_dir'], csv_training[i]),
                                           root_dir=d['data_dir'], transform_face=data_transform_face[x],
                                           transform_pose=data_transform_pose[x])
                   for i, x in enumerate(['train', 'eval'])}
    elif d['model'] == 'cnn_lstm_vision_audio':
        dataset = {x: CustomDataset_vision_audio(label_dir=os.path.join(d['data_dir'], csv_training[i]),
                                           root_dir=d['data_dir'], transform_face=data_transform_face[x],
                                           transform_pose=data_transform_pose[x],
                                           transform_spect=data_transform_spect[x])
                   for i, x in enumerate(['train', 'eval'])}
    elif d['model'] == 'cnn_audio':
        dataset = {x: CustomDataset_audio_cnn(label_dir=os.path.join(d['data_dir'], csv_training[i]),
                                              root_dir=d['data_dir'], transform_spect=data_transform_spect[x])
                   for i, x in enumerate(['train', 'eval'])}
    elif d['model'] == 'cnn_lstm_audio':
        dataset = {x: CustomDataset_audio(label_dir=os.path.join(d['data_dir'], csv_training[i]),
                                          root_dir=d['data_dir'], transform_spect=data_transform_spect[x])
                   for i, x in enumerate(['train', 'eval'])}
    ...
    if d['model'] == 'cnn_audio':
        dataloader = {x: DataLoader(dataset[x], batch_size=d['n_seq'], shuffle=False, num_workers=0, worker_init_fn=seed_worker,
    generator=g)
                      for x in ['train', 'eval']}
    else:
...
def data_loader_test(d, test_filename):
    ...
    if d['model'] == 'cnn_lstm_mm':
        ...
        dataset = CustomDataset_vision_with_other(label_dir=os.path.join(d['data_dir'], test_filename),
                                       root_dir=d['data_dir'], transform_face=data_transform_face['test'],
                                       transform_pose=data_transform_pose['test'])
    elif d['model'] == 'cnn_lstm_vision_audio':
        dataset = CustomDataset_vision_audio(label_dir=os.path.join(d['data_dir'], test_filename),
                                             root_dir=d['data_dir'], transform_face=data_transform_face['test'],
                                             transform_pose=data_transform_pose['test'],
                                             transform_spect=data_transform_spect['test'])
    elif d['model'] == 'cnn_audio':
        dataset = CustomDataset_audio_cnn(label_dir=os.path.join(d['data_dir'], test_filename),
                            root_dir=d['data_dir'], transform_spect=data_transform_spect['test'])
    elif d['model'] == 'cnn_lstm_audio':
        dataset = CustomDataset_audio(label_dir=os.path.join(d['data_dir'], test_filename),
                            root_dir=d['data_dir'], transform_spect=data_transform_spect['test'])
    elif d['model'] == 'cnn_lstm_image':
        dataset = CustomDataset_vision(label_dir=os.path.join(d['data_dir'], test_filename),
                            root_dir=d['data_dir'], transform_face=data_transform_face['test'],
    ...
    if d['model'] == 'cnn_audio':
        dataloader = DataLoader(dataset, batch_size=d['n_seq'], shuffle=False, num_workers=0, worker_init_fn=seed_worker,
    generator=g)
    else:
...
def data_loader_test_solo(d, test_filename):
    ...
    if d['model'] == 'cnn_lstm_mm':
        ...
        dataset = CustomDataset_vision_with_other(label_dir=os.path.join(d['labels_dir'], test_filename),
                                       root_dir=d['data_dir'], transform_face=data_transform_face['test'],
                                       transform_pose=data_transform_pose['test'])
    elif d['model'] == 'cnn_lstm_vision_audio':
        dataset = CustomDataset_vision_audio(label_dir=os.path.join(d['labels_dir'], test_filename),
                                             root_dir=d['data_dir'], transform_face=data_transform_face['test'],
                                             transform_pose=data_transform_pose['test'],
                                             transform_spect=data_transform_spect['test'])
    elif d['model'] == 'cnn_audio':
        dataset = CustomDataset_audio_cnn(label_dir=os.path.join(d['labels_dir'], test_filename),
                            root_dir=d['data_dir'], transform_spect=data_transform_spect['test'])
    elif d['model'] == 'cnn_lstm_audio':
        dataset = CustomDataset_audio(label_dir=os.path.join(d['labels_dir'], test_filename),
                            root_dir=d['data_dir'], transform_spect=data_transform_spect['test'])
    elif d['model'] == 'cnn_lstm_image':
        dataset = CustomDataset_vision(label_dir=os.path.join(d['labels_dir'], test_filename),
                            root_dir=d['data_dir'], transform_face=data_transform_face['test'],
