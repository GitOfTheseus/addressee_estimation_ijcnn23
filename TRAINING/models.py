...
class Conv_NN_Image(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_layer2 = nn.Conv2d(in_channels=6, out_channels=6, kernel_size=5)  # 2
        self.relu2 = nn.LeakyReLU()
        self.max_pool1 = nn.MaxPool2d(kernel_size=2, stride=2)  # 3

        self.conv_layer3 = nn.Conv2d(in_channels=6, out_channels=8, kernel_size=5)  # 4
        self.relu3 = nn.LeakyReLU()
        self.conv_layer4 = nn.Conv2d(in_channels=8, out_channels=8, kernel_size=3)  # 5
        self.relu4 = nn.LeakyReLU()
        self.max_pool2 = nn.MaxPool2d(kernel_size=2, stride=2)  # 6

        self.fc1 = nn.Linear(9248, 1156)  # 7
        self.relu5 = nn.LeakyReLU()  # 8
        self.fc2 = nn.Linear(1156, 578)  # 9
...
class Conv_NN_Image_a(nn.Module):
    #  Determine what layers and their order in CNN object
    def __init__(self):
        super(Conv_NN_Image, self).__init__()

        self.conv_layer1 = nn.Conv2d(in_channels=3, out_channels=6, kernel_size=5)  # 1
        self.relu1 = nn.ReLU()
        self.max_pool1 = nn.MaxPool2d(kernel_size=2, stride=2)  # 3

        self.conv_layer2 = nn.Conv2d(in_channels=6, out_channels=6, kernel_size=7)  # 2
        self.relu2 = nn.ReLU()
        self.max_pool2 = nn.MaxPool2d(kernel_size=2, stride=2)  # 3

        self.conv_layer3 = nn.Conv2d(in_channels=6, out_channels=8, kernel_size=9)  # 4
        self.relu3 = nn.ReLU()
        self.max_pool3 = nn.MaxPool2d(kernel_size=2, stride=2)  # 3

        self.fc1 = nn.Linear(1568, 578)  # 7
        self.relu5 = nn.ReLU()  # 8
        #self.fc2 = nn.Linear(1156, 578)  # 9

    # Progresses data across layers
    def forward(self, x):
        #print("0 face", x.shape)
        out = self.conv_layer1(x)
        #print("1", out.shape)
        out = self.relu1(out)
        #print("1", out.shape)
...
# CNN model for poses
class Conv_NN_Pose_a(nn.Module):
    #  Determine what layers and their order in CNN object
    def __init__(self):
        super(Conv_NN_Pose, self).__init__()

        self.conv_layer1 = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(3, 1))
        self.relu1 = nn.ReLU()
        self.max_pool1 = nn.MaxPool2d(kernel_size=(2, 1), stride=(2, 1))

        self.conv_layer2 = nn.Conv2d(in_channels=8, out_channels=16, kernel_size=(2, 1))
        self.relu2 = nn.ReLU()
        self.max_pool2 = nn.MaxPool2d(kernel_size=(2, 1), stride=(2, 1))

        self.conv_layer3 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=(2, 1))
        self.relu3 = nn.ReLU()
    ...
    def forward(self, x):
        ...
        #print("fc1", out.shape)
        out = self.relu5(out)
        #print("fc1 pose", out.shape)

        return out


# Creating a LSTM class
class LSTM_NN_Vision_a(nn.Module):
    #  Determine what layers and their order in CNN object
    def __init__(self, hidden_dim, seq_len, layer_dim, n_seq, num_classes, device):
        super(LSTM_NN_Vision, self).__init__()
...
def build_models(d):
    ...
    return cnn_image_d, cnn_pose_d, cnn_audio_d, lstm_vision_d, lstm_vision_with_other_d, lstm_vision_audio_d, lstm_audio_d, lstm_image_d, lstm_pose_d
